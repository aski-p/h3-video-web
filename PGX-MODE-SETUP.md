...
6. Once idle, as `aski`, run `systemctl --user daemon-reload` and restart `h3-web-backend.service`. Ensure the actual backend unit has the drop-in. Confirm `/api/pgx-mode` through the existing authorized proxy. Qwen readiness now requires `/v1/models` to return `Qwen3.8-Flash-Next-EXL3`, and the mode response reports its `262144` context and `127.0.0.1:8899/v1` route. Do not expose the origin token or bypass origin authentication.
7. As root, run `python3 scripts/configure-hermes-qwen38.py --home /home/aski/hermes-official --restart`. It preserves a timestamped config backup, switches Hermes from the retired Qwen route to the Flash Next EXL3 route, pins `262144`, clears stale context metadata, and restarts only the Hermes gateway.
8. Update the actual installed Windows worker's shared `server.py`, new `pgx_mode.py`, and `h3_worker.py` from `windows-worker/`. Back up first; preserve config and encrypted token. Restart only the worker task when idle. New install packages must include `pgx_mode.py` (installer file list was updated).
9. Verify PIN rejection, PGX busy rejection, video → Qwen (`/v1/models` and one short completion), and Qwen → video (`/system_stats`). Check there is never simultaneous GPU residency from these two services. The UI must never show ready while still loading.
10. On RTX 5080 run one image-reference 10-second continuous sample first; verify output duration and frame count, then 15 seconds. Single-take cap is now 15 seconds instead of forcing >5 seconds into split segments. I2V maximum remains 15 seconds. Longer T2V retains splitting. Worker timeout scales from 6 hours at 124 frames up to 24 hours; cancellation/lease checks remain active. This does not guarantee that 16 GB VRAM can fit every resolution/LoRA combination; report OOM honestly and do not silently shorten the video.
11. Report actual service states, model revision, successful tests, and remaining blocks. Repository tests are not hardware validation.
