...
for path in paths:
 ...
 for idx in indices:
  ...
  if not faces: scores.append(None);tiles.append(np.zeros((280,240,3),np.uint8));continue
  f=faces[0];scores.append(float(np.dot(f.embedding_norm,source_face.embedding_norm)))
