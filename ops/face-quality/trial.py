...
def main():
    p=argparse.ArgumentParser();p.add_argument('--engine',type=Path,required=True);p.add_argument('--source',type=Path,required=True);p.add_argument('--portrait',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--model',choices=['ghost_1_256','hyperswap_1a_256','hyperswap_1b_256','hyperswap_1c_256'],required=True);p.add_argument('--reference-distance',type=float,default=.3);p.add_argument('--selector-mode',choices=['reference','one'],default='reference');p.add_argument('--detector-score',type=float,default=.5);p.add_argument('--tone-from',type=Path);p.add_argument('--reference-frame',type=int,default=30);p.add_argument('--expression',type=int,default=0);a=p.parse_args()
    ...
    sys.argv=['facefusion.py','headless-run','-s',str(a.portrait),'-t',str(a.source),'-o',str(a.output),'--processors',*processors,
      ...
      '--face-selector-mode',a.selector_mode,'--face-selector-order','large-small','--face-detector-score',str(a.detector_score),'--reference-frame-number',str(a.reference_frame),'--reference-face-position','0','--reference-face-distance',str(a.reference_distance),'--face-selector-gender','female',
